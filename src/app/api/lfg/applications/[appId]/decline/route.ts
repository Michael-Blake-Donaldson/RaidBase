...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (application.post.creatorId !== user.id) {
      return fail("FORBIDDEN", "You can only decline applications on your own posts.", 403);
    }
    ...
    await createUserNotification({
      ...
      type: "LFG_REQUEST_DECLINED",
      title: "Your join request was declined",
      body: `Unfortunately, your application for "${application.post.title}" was not selected. Check out other available posts.`,
      href: "/lfg",
    });
